...
class TSPTW():
    def __init__(self, filename: str):
        ...
        with open(filename, "r") as f:
            ...
            node_attributes = {index + 1: {"index": index+1} for index in range(self.num_nodes)}
    ...
    def display_solution(self, solution: List[int], plot_name: str = ""):
        ...
        edge_labels = dict([((u, v,), f'{int(d["weight"])}\n\n{int(self.graph.edges[(v,u)]["weight"])}')
                        for u, v, d in self.graph.edges(data=True) if pos[u][0] > pos[v][0]])
        nx.draw_networkx_edge_labels(self.graph, pos=pos, edge_labels=edge_labels, label_pos=0.3)
        nx.draw_networkx_edges(
            self.graph,
            pos,
        ...
        nx.draw_networkx_nodes(self.graph,  pos=pos, nodelist=[0], node_color="tab:green")
