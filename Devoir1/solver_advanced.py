...
# Function to return the best solution after all the restarts are completed 
        ...
        with time_limit(TIME_LIMIT):
            for i in range(n_restart):
                ...
                try:
                    ...
                    print(
                        f"Random.choice Indexation error in build initial solution for {i+1}"
                    )
                    continue
...
def solver(pcstp: PCSTP) -> List[Tuple[int]]:
    ...
    try:
        while True:
            ...
            if nb_try % 1000 == 0:
                print(
                    f"{nb_try}/{nb_try_in_batch} - {solution_changed_in_batch} - {current_score}"
                )

            if nb_try >= nb_try_in_batch:
                if not solution_changed_in_batch:
                    print(f"Number of batches done: {nb_batch_done}")
                    break
                else:
                    # ? Reduce batch size
...
def find_better_local_solution(node: Node, pcstp: PCSTP) -> Tuple[Node, bool]:
    ...
    heuristic = random.choice(
        [
            small_neighborhood_heuristic,
            large_neighborhood_heuristic,
            terminal_node_heuristic,
        ]
    )
    ...
    for adj_node_id in pcstp.network.adj[root.id]:
        ...
        if adj_node_id not in nodes_id:
            ...
            if new_score < current_score or random.random() > 0.975:  # * stochasticity
                change_made, connections, nodes_id, current_score = (
                    True,
                    new_connections,
                    new_nodes_id,
                    new_score,
